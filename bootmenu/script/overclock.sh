...
install_module()
{
  ...
  if [ $load_all -eq 1 ]; then
	...
	insmod $MODULE_DIR/cpufreq_smartass2.ko
...
set_scaling()
{
  case "$scaling" in
    "0" )
      ...
      echo $con_sampling_rate > /sys/devices/system/cpu/cpufreq/conservative/sampling_rate
      echo $con_freq_step > /sys/devices/system/cpu/cpufreq/conservative/freq_step
      echo $con_up_threshold > /sys/devices/system/cpu/cpufreq/conservative/up_threshold
      echo $con_down_threshold > /sys/devices/system/cpu/cpufreq/conservative/down_threshold
    ;;
    "1" )
      if [ $load_all -eq 0 ]; then
        insmod $MODULE_DIR/symsearch.ko
	...
	"2" )
      ...
      echo $int_min_sample_rate > /sys/devices/system/cpu/cpufreq/interactiveX/min_sample_time
    ;;
    "3" )
